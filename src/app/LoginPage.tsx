...
export function LoginPage() {
  ...
  return <Box sx={{ minHeight: '100vh', display: 'grid', placeItems: 'center', p: 2, background: 'radial-gradient(circle at 15% 15%, #377293 0, transparent 32%), linear-gradient(135deg, #102d45 0%, #173c5c 48%, #1b6659 145%)' }}><Paper component="form" onSubmit={submit} elevation={0} sx={{ p: { xs: 3, sm: 4.5 }, width: '100%', maxWidth: 430, border: '1px solid rgba(255,255,255,.42)', boxShadow: '0 24px 70px rgba(4, 20, 33, .32)' }}><Stack spacing={2.4}>
    <Stack direction="row" spacing={1.5} alignItems="center"><Avatar sx={{ bgcolor: 'secondary.main', color: 'primary.dark', width: 44, height: 44 }}><AccountTreeOutlinedIcon /></Avatar><Box><Typography variant="h5">Cosechador</Typography><Typography color="text.secondary" variant="body2">Administración v5 de LA Referencia</Typography></Box></Stack>
    <Typography color="text.secondary">Accede a la operación de fuentes, acciones y configuraciones de la instalación.</Typography>
    ...
    <Button type="submit" variant="contained" size="large" disabled={pending} startIcon={<LockOutlinedIcon />}>Ingresar</Button>
  </Stack></Paper></Box>
}
